...
test_that("parse_population processes population data correctly", {
  ...
  expect_equal(body(fun=expected_output$random_variate_fun), body(calc_output$random_variate_fun))
...
testthat::test_that("Both populations normally distributed, equal, centered and normalized", {
  ...
  calc <- process_input(here("data","test","input_2_std_normal.yaml"), 
...
testthat::test_that("Both populations normally distributed, one standard, the other centered and sd=10", {
  ...
  calc <- process_input(here("data","test","input_2_normal_diff_sd.yaml"),
                        num.simulations = num.simulations, 
                        seed = a.seed)
...
testthat::test_that("Both populations t-student distributed and 4 degrees of freedom", {
  ...
  calc <- process_input(here("data","test","input_2_t_4df.yaml"),
                        num.simulations = num.simulations,
                        seed = a.seed)
...
testthat::test_that("Both populations exponentially distributed with rate 4", {
  ...
  num.simulations <- 3
  a.seed <- 1234
  calc <- process_input(here("data","test","input_2_exp_rate_4.yaml"),
                        num.simulations = num.simulations, 
                        seed = a.seed)
...
testthat::test_that("One population normally distributed the other exponential and rate 0.1", {
  ...
  num.simulations <- 3
  a.seed <- 1234
  calc <- process_input(here("data","test","input_normal_exp.yaml"), 
                        num.simulations = num.simulations,
                        seed = a.seed)               
